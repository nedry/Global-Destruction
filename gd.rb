...
class GD_thread
  ...
  def run
    ...
    startup = false
    @console.puts "-Starting GD thread for channel: #{@c_num}"
    @console.puts "-Adding virtual players for channel: #{@c_num}"
    
    #v_add(2)
    v_create_list(2, @console)
    loop do
      do_tick 
      sleep(0.1)
      # @console.puts "GDthread: #{@started}"
      ...
      if !@started then 
        @channel[@c_num].g_buffer.each {|x| 
          parray 	= Array.new
          @console.puts "x: #{x}"
          @console.puts "x.split: #{x.split.each {|x| put x}}"
          parray	= x.split
          @console.puts "DEBUG: players.length: #{@players.len}"
          @console.puts "DEBUG: parray: #{parray.length}"
          @console.puts "DEBUG: parray: #{parray.each {|x| @console.puts x}}"	
          if parray.length > 1 then
            if parray[1].upcase == "START"  and !@started then
              sleep(1)
              @console.puts "DEBUG: -resetting game"
              reset 
              #v_add(2)
              @console.puts "DEBUG: vplayers.length: #{@v_players.len}"
              v_combine
              @console.puts "DEBUG: players.length: #{@players.len}"
              @console.puts "DEBUG: parray: #{parray.length}"
              @console.puts "DEBUG: parray: #{parray.each_with_index {|x,i| @console.puts i,x}}"			
              startit(parray[0])
              @channel[@c_num].status = true
              
              
              
              @console.puts "-Starting GD Game"
            end
          end
