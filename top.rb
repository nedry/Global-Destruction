...
class Session 
  ...
  def initialize(users, who, channel, log, socket, console )
    @console = console
    @socket  = socket 
    @users  = users
    @who  = who
...
class Happythread
  ...
  def removechannel(name)
    x = 0
    f = false
    @console.puts "-Removing #{name} from all chat channels." 
    @channel.each {|chan|	
    ...
    while true
      ...
      each_name_with_index {|name, i|
        if !curthread.any? {|thr| @who[i].threadn == thr}
          @console.puts "-User #{i}:#{name} has disconnected."
...
class ServerSocket
  ...
  def run
    ...
    Thread.new {Happythread.new(@users,@who,@channel,@log, @console).run}
    ...
    while true
      if socket = @serverSocket.accept then
        Thread.new {
          ...
          Session.new(@users,@who,@channel,@log,socket,@console).run
        }
      end
    end
