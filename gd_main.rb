  ...
  number.times {
    while true
      ...
      if @usd_country.index(nextp) == nil then
        ...
        puts "DEBUG:Adding: #{VNAMES[nextp]}"
...
def v_create_list (number, console)
  number.times {
    while true
      nextp = rand(VCOUNT.length)
      if @usd_country.index(nextp) == nil then
        @v_players.append(Player.new(VNAMES[nextp],VNAMES[nextp],VCOUNT[nextp],true,@deck.choosehand))
        console.puts "-Adding: #{VNAMES[nextp]}"
        @channel[@c_num].v_usrchannel.push(VNAMES[nextp])   #add them to the virtual channel in teleconference
        @usd_country.push(nextp)
        break
      else console.puts "DEBUG:Collision... try again..#{nextp}" end
    end }
end
...
def show_all(u)
  ...
  for i in 0..@players.len - 1 do
    blat_usr(u," %R#{@players[i].name} %Wcommander of %Y#{@players[i].country} %Wwith%R #{@players[i].population} %Wmillion folks.")
  end
  blat_usr(u,"***")
end
...
def handlebombdrop(owner,user,hits)
  ...
  if detonate then
    ...
    adjective = NUKILL[rand(NUKILL.length - 1)]
    blat_usr(user,"%R*** %WA warhead from %Y#{@players[owner].country}%W detonates over your country, #{adjective} %R#{dead} million %Wpeople!")
...
def get_target(user,parray,suppress)
  ...
  if parray.length > 0 then
    ...
    # target = parse_country(user,parray[j])
    target = parse_country(user,parray.last)
    #break if target != nil
  end
  
  blat_usr(user,"%W...There is no country by that name at this table!") if target.nil? and !suppress
  ...
  if target == -2 then
    blat_usr(user,"%RYou need to be more specific about which country you mean.") if not !suppress
    target = nil
  end
...
def do_tick
  ...
  if @started
    ...
    if @complete then
      ...
      u = @players[@turn].name
      puts "TURN: #{u}"
      @channel[@c_num].a_player = u  #The chat server needs to know the current player
      @channel[@c_num].locked = false #The chat server needs to know if the input is locked
      ...
      #The chat server needs a list of active players for channel locking...
      i=0; @channel[@c_num].players = []
      
      for i in 0..@players.len - 1
        if !@players[i].virtural and !@players[i].dead then
          @channel[@c_num].players <<  @players[i].name 
        end
      end
